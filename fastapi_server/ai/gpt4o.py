...
# 🔹 Единоразовый запрос
@router.post("/ask_gpt4o")
async def ask_gpt4o(prompt: str, token: str, max_dollars: float, temperature: float = 0.3):
    db = DatabaseAdapter()
    db.connect()

    balance = get_token_balance(db, token)
    max_cost = estimate_chatgpt4o_total_cost(prompt=prompt) + max_dollars
    if max_cost > balance:
        raise HTTPException(status_code=402, detail=f"Недостаточно средств: {balance} < {max_cost}")
...
async def chat_with_dialog(payload: DialogQuery):
    ...
    rough_cost = estimate_chatgpt4o_total_cost(prompt_text)

    if rough_cost > balance + payload.max_dollars:
